...
class SellOrder(models.Model):
    ...
    so_identify = models.CharField(max_length=15, verbose_name='销售订单编号')
    so_serial = models.CharField(max_length=4, verbose_name='流水号')
    organization = models.ForeignKey('base.Organization', verbose_name='组织', related_name='org_so', on_delete=models.CASCADE)
    so_type = models.IntegerField(choices=SELL_ORDER_CHOICES, verbose_name='订单类型')
    customer = models.ForeignKey('base.Customer', verbose_name='客户', related_name='customer_so', on_delete=models.CASCADE)
    so_date = models.DateTimeField(default=datetime.datetime.now, verbose_name='订单日期')
    deliver_ware_house = models.CharField(max_length=20, verbose_name='发货仓库名字')
    # deliver_ware_house_identify = models.CharField(max_length=6, verbose_name='发货仓库编码')
    ...
    so_creator_identify = models.CharField(max_length=20, verbose_name='订单创建人编码')
    so_created_at = models.DateTimeField(auto_now_add=True, verbose_name='订单创建日期')

    class Meta:
        db_table = 'db_sellOrder'
        verbose_name = "销售订单"
...
class SellOrderDetail(models.Model):
    ...
    material = models.ForeignKey('base.Material', verbose_name='物料', related_name='material_sod', on_delete=models.CASCADE)
